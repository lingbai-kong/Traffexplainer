...
def run_model(task=None, model_name=None, dataset_name=None, config_file=None,
    ...
    if model_name == 'Explainer':
        assert 'base_model' in other_args
        Mconfig = ConfigParser(task, other_args['base_model'], dataset_name,
                          config_file, saved_model, train, other_args)
    ...
    if exp_id is None:
        ...
        if 'Mconfig' in config:
            config['Mconfig']['exp_id'] = exp_id
    # logger
    ...
    # 加载数据集
    if model_name == 'Explainer':
        dataset = get_dataset(Mconfig)
        logger.info(Mconfig.config)
    else:
    ...
    if model_name == 'Explainer':
        executor = get_executor(Mconfig, model)
    else:
