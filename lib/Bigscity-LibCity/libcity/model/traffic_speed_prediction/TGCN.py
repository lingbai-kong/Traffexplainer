...
def calculate_normalized_laplacian(adj,device):
    """
    ...
    Returns:
        np.ndarray: L
    """
    d = adj.sum(1).to(device)
    d_inv_sqrt = torch.pow(d, -0.5).flatten().to(device)
    d_inv_sqrt[torch.isinf(d_inv_sqrt)] = 0.
    d_mat_inv_sqrt = torch.diag(d_inv_sqrt).to(device)
    normalized_laplacian = torch.eye(adj.shape[0]).to(device) - adj.mm(d_mat_inv_sqrt).transpose(1,0).mm(d_mat_inv_sqrt)
...
class TGCNCell(nn.Module):
    def __init__(self, num_units, adj_mx, num_nodes, device, input_dim=1):
        ...
        # 这里提前构建好拉普拉斯
        self.normalized_adj = self.normalized_adj1 = self.normalized_adj2 = calculate_normalized_laplacian(adj_mx,self._device)
        self.init_params()
    ...
    def forward(self, inputs, state):
        ...
        value = torch.sigmoid(
            self._gc(inputs, state, output_size, self.normalized_adj1, bias_start=1.0))  # (batch_size, self.num_nodes, output_size)
        ...
        c = self.act(self._gc(inputs, r * state, self.num_units, self.normalized_adj2))
        c = c.reshape(shape=(-1, self.num_nodes * self.num_units))
        new_state = u * state + (1.0 - u) * c
        return new_state

    def _gc(self, inputs, state, output_size, normalized_adj, bias_start=0.0):
        """
        GCN
        ...
        x1 = torch.mm(normalized_adj.float(), x0.float())  # A * X
...
class TGCN(AbstractTrafficStateModel):
    ...
    def updateAdj(self, adj_mx1, adj_mx2):
        self.tgcn_model.normalized_adj1 = calculate_normalized_laplacian(adj_mx1, self.device)
        self.tgcn_model.normalized_adj2 = calculate_normalized_laplacian(adj_mx2, self.device)
