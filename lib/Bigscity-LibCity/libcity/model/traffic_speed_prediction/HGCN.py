...
class HGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.init_weight_inf_or_zero = self.data_feature.get('init_weight_inf_or_zero', 'inf')
        self.weight_adj_epsilon = self.data_feature.get('weight_adj_epsilon')
        ...
        self.supports = self.supports1 = self.supports2 = [torch.tensor(i).to(self.device) for i in self.adj_mx]
        self.supports_cluster = self.supports_cluster1 = self.supports_cluster2 = [torch.tensor(i).to(self.device) for i in self.adj_mx_cluster]
        # self.supports = self.supports1 = self.supports2 = [torch.tensor(self.adj_mx)]
        # self.supports_cluster = self.supports_cluster1 = self.supports_cluster2 = [self.adj_mx_cluster.clone().detach()]
    ...
    def forward(self, batch):
        ...
        new_supports1 = self.supports1 + [A]
        new_supports2 = self.supports2 + [A]
        # region
        ...
        new_supports_cluster1 = self.supports_cluster1 + [A_cluster]
        new_supports_cluster2 = self.supports_cluster2 + [A_cluster]
        ...
        # 1
        x_cluster = self.block_cluster1(x_cluster, new_supports_cluster1)
        ...
        # 2
        x_cluster = self.block_cluster2(x_cluster, new_supports_cluster2)
        x = self.block2(x, new_supports2)
    ...
    def calculate_adj_mx_cluster(self, adj_mx):
        '''
        :return: #聚类结果[cluster_num][]   聚类标识[cluster_num][]
        '''
        adj_mx_cluster = torch.zeros((self.cluster_nodes, self.cluster_nodes), dtype=torch.float).to(self.device)
        for i in range(self.cluster_nodes):
            for j in range(self.cluster_nodes):
                tmp1,tmp2=torch.tensor(self.centers_ind_groups[i]).to(self.device), torch.tensor(self.centers_ind_groups[j]).to(self.device)
                tmp1_len, tmp2_len=tmp1.shape[0], tmp2.shape[0]
                tmp1=tmp1.expand(tmp2_len,-1).reshape(-1)
                tmp2=tmp2.expand(tmp1_len,-1).T.reshape(-1)
                cluster_sum = torch.sum(adj_mx[tmp1,tmp2])
                adj_mx_cluster[i][j] = cluster_sum
                tmp1,tmp2 = tmp1.cpu(),tmp2.cpu()
        distances = adj_mx_cluster[~torch.isinf(adj_mx_cluster)].flatten()
        std = distances.std()
        adj_mx_cluster = torch.exp(-torch.square(adj_mx_cluster / std))
        adj_mx_cluster_mask = torch.ones_like(adj_mx_cluster ,dtype=torch.float).to(self.device)
        adj_mx_cluster_mask[adj_mx_cluster < 0.1] = 0
        return adj_mx_cluster * adj_mx_cluster_mask
    
    def updateAdj(self, adj_mx1, adj_mx2):
        self.supports1 = [i for i in adj_mx1]
        self.supports2 = [i for i in adj_mx2]
        self.supports_cluster1 = [i for i in self.calculate_adj_mx_cluster(adj_mx1)]
        self.supports_cluster2 = [i for i in self.calculate_adj_mx_cluster(adj_mx2)]
