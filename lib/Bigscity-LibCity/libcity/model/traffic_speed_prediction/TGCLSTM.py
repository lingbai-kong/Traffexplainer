...
class TGCLSTM(AbstractTrafficStateModel):
    ...
    def updateAdj(self, adj_mx1, adj_mx2):
        adj_temp = torch.eye(self.num_nodes, self.num_nodes, device=self.device)
        adj_temp1 = torch.matmul(adj_temp, adj_mx1)
        adj_temp1 = torch.matmul(adj_temp, adj_mx2)
        if self.config.get('Clamp_A', True):
            # confine elements of A
            adj_temp1 = torch.clamp(adj_temp1, max=1.)
            adj_temp2 = torch.clamp(adj_temp2, max=1.)
        if self.dataset_class == "TGCLSTMDataset":
            adj1 = torch.mul(adj_temp1, torch.Tensor(self.data_feature['FFR'][self.back_length]).to(self.device))
            adj2 = torch.mul(adj_temp2, torch.Tensor(self.data_feature['FFR'][self.back_length]).to(self.device))
        else:
            adj1 = adj_temp1
            adj2 = adj_temp2
        self.gc_list[0].filter_square_matrix = adj1.repeat(self.output_dim, self.input_dim).to(self.device)
        self.gc_list[1].filter_square_matrix = adj2.repeat(self.output_dim, self.input_dim).to(self.device)
