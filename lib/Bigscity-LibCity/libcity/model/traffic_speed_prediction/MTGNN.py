...
class MTGNN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.predefined_A = self.predefined_A1 = self.predefined_A2 = self.predefined_A.to(self.device)
    ...
    def forward(self, batch, idx=None):
        ...
        adps = [self.predefined_A1, self.predefined_A2]
        for i in range(len(adps),self.layers):
            adps.append(adp)
        ...
        for i in range(self.layers):
            ...
            skip = s + skip
            if self.gcn_true:
                x = self.gconv1[i](x, adps[i])+self.gconv2[i](x, adps[i].transpose(1,0))
            else:
    ...
    def updateAdj(self, adj_mx1, adj_mx2):
        self.predefined_A1 = adj_mx1
        self.predefined_A2 = adj_mx2
