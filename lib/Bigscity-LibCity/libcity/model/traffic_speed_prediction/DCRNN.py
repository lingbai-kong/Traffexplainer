...
def calculate_random_walk_matrix(adj_mx):
    d = adj_mx.detach().sum(1)
    d_inv = torch.pow(d, -1).flatten()
    d_inv[torch.isinf(d_inv)] = 0.
    d_mat_inv = torch.diag(d_inv)
    random_walk_mx = adj_mx.mm(d_mat_inv)
    ...
    def forward(self, inputs, state):
        ...
        if self._max_diffusion_step == 0:
            ...
            for support in self._supports:
                # T1=L x1=T1*x=L*x
                x1 = torch.mm(support, x0)  # supports: n*n; x0: n*(total_arg_size * batch_size)
...
#             self._supports.append(self._build_sparse_matrix(support, self._device))
        ...
        if self._use_gc_for_ru:
            self._fn = GCONV(self._num_nodes, self._max_diffusion_step, self.supports, self._device,
        ...
        self._gconv = GCONV(self._num_nodes, self._max_diffusion_step, self.supports, self._device,
...
class DCRNN(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        self.device = config.get('device', torch.device('cpu'))
        self.adj_mx = torch.from_numpy(data_feature['adj_mx']).to(self.device) 
...
#         print('adj_mx1:',adj_mx1)
#         print('random_walk:',calculate_random_walk_matrix(adj_mx1).T)
        
        supports1=self.genSupports(adj_mx1)
        supports2=self.genSupports(adj_mx2)

        for i in range(len(self.encoder_model.dcgru_layers)):
            self.encoder_model.dcgru_layers[i]._gconv._supports = supports1
            self.encoder_model.dcgru_layers[i]._gconv._num_matrices = len(supports1) * self.max_diffusion_step + 1
        for i in range(len(self.decoder_model.dcgru_layers)):
            self.decoder_model.dcgru_layers[i]._gconv._supports = supports2
            self.decoder_model.dcgru_layers[i]._gconv._num_matrices = len(supports2) * self.max_diffusion_step + 1
