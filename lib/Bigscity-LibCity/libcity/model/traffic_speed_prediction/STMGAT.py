...
class STMGAT(AbstractTrafficStateModel):
    ...
    def _get_adj(self, adj_mx=None):
        if adj_mx==None:
            adj_mx = torch.tensor(self.data_feature.get('adj_mx', 1))
        edge_list = []
        possitive_list=torch.nonzero(adj_mx>0)
        for e in possitive_list:
            edge_list.append((e[0], e[1], adj_mx[e[0]][e[1]]))
    ...
    def updateAdj(self, adj_mx1, adj_mx2):
        self.g = self._get_adj(adj_mx1)
        # self.g2 = self._get_adj(adj_mx2)
