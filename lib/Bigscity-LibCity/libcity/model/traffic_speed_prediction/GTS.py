...
class GTS(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        self.adj_mx = self.adj_mx1 = self.adj_mx2 = torch.Tensor(data_feature.get('adj_mx')).to(self.device)
        ...
        # graph structure learner for encoder
        self.conv1_1 = torch.nn.Conv1d(1, 8, self.kernal_size, stride=1)
        self.conv2_1 = torch.nn.Conv1d(8, 16, self.kernal_size, stride=1)
        self.hidden_drop_1 = torch.nn.Dropout(0.2)
        self.fc_1 = torch.nn.Linear(self.dim_fc, self.embedding_dim)
        self.bn1_1 = torch.nn.BatchNorm1d(8)
        self.bn2_1 = torch.nn.BatchNorm1d(16)
        self.bn3_1 = torch.nn.BatchNorm1d(self.embedding_dim)
        self.fc_out_1 = nn.Linear(self.embedding_dim * 2, self.embedding_dim)
        self.fc_cat_1 = nn.Linear(self.embedding_dim, 2)

        # graph structure learner for decoder
        self.conv1_2 = torch.nn.Conv1d(1, 8, self.kernal_size, stride=1)
        self.conv2_2 = torch.nn.Conv1d(8, 16, self.kernal_size, stride=1)
        self.hidden_drop_2 = torch.nn.Dropout(0.2)
        self.fc_2 = torch.nn.Linear(self.dim_fc, self.embedding_dim)
        self.bn1_2 = torch.nn.BatchNorm1d(8)
        self.bn2_2 = torch.nn.BatchNorm1d(16)
        self.bn3_2 = torch.nn.BatchNorm1d(self.embedding_dim)
        self.fc_out_2 = nn.Linear(self.embedding_dim * 2, self.embedding_dim)
        self.fc_cat_2 = nn.Linear(self.embedding_dim, 2)
    ...
    def forward(self, batch, batches_seen=None):
        ...
        # 图结构的推断过程
        ## for encoder
        x1 = self.node_feas.transpose(1, 0).view(self.num_nodes, 1, -1)  # [207, 1, 24000]
        x1 = self.conv1_1(x1)  # [207, 8, 23991]
        x1 = F.relu(x1)
        x1 = self.bn1_1(x1)
        # x = self.hidden_drop(x)
        x1 = self.conv2_1(x1)  # [207, 16, 23982]
        x1 = F.relu(x1)
        x1 = self.bn2_1(x1)
        x1 = x1.view(self.num_nodes, -1)  # [207, 383712]
        x1 = self.fc_1(x1)
        x1 = F.relu(x1)
        x1 = self.bn3_1(x1)

        receivers1 = torch.matmul(self.rel_rec, x1)
        senders1 = torch.matmul(self.rel_send, x1)
        x1 = torch.cat([senders1, receivers1], dim=1)
        x1 = torch.relu(self.fc_out_1(x1))
        x1 = self.fc_cat_1(x1)

        adj1 = gumbel_softmax(self.device, x1, temperature=self.temperature, hard=True)
        adj1 = adj1[:, 0].clone().reshape(self.num_nodes, -1)
        mask = torch.eye(self.num_nodes, self.num_nodes).bool().to(self.device)
        adj1.masked_fill_(mask, 0)
        adj1=adj1*self.adj_mx1

        ## for decoder
        x2 = self.node_feas.transpose(1, 0).view(self.num_nodes, 1, -1)  # [207, 1, 24000]
        x2 = self.conv1_2(x2)  # [207, 8, 23991]
        x2 = F.relu(x2)
        x2 = self.bn1_2(x2)
        # x = self.hidden_drop(x)
        x2 = self.conv2_2(x2)  # [207, 16, 23982]
        x2 = F.relu(x2)
        x2 = self.bn2_2(x2)
        x2 = x2.view(self.num_nodes, -1)  # [207, 383712]
        x2 = self.fc_2(x2)
        x2 = F.relu(x2)
        x2 = self.bn3_2(x2)

        receivers2 = torch.matmul(self.rel_rec, x2)
        senders2 = torch.matmul(self.rel_send, x2)
        x2 = torch.cat([senders2, receivers2], dim=1)
        x2 = torch.relu(self.fc_out_2(x2))
        x2 = self.fc_cat_2(x2)

        adj2 = gumbel_softmax(self.device, x2, temperature=self.temperature, hard=True)
        adj2 = adj2[:, 0].clone().reshape(self.num_nodes, -1)
        mask = torch.eye(self.num_nodes, self.num_nodes).bool().to(self.device)
        adj2.masked_fill_(mask, 0)
        adj2=adj2*self.adj_mx2
        encoder_hidden_state = self.encoder(inputs, adj1)
        self._logger.debug("Encoder complete, starting decoder")
        outputs = self.decoder(encoder_hidden_state, adj2, labels, batches_seen=batches_seen)
        ...
        return orig_out, x1[:, 0].clone().reshape(self.num_nodes, -1), x2[:, 0].clone().reshape(self.num_nodes, -1)
    ...
    def calculate_loss(self, batch, batches_seen=None):
        ...
        y_predicted, mid_output1, mid_output2 = self.forward(batch, batches_seen)
        ...
        if epoch < self.epoch_use_regularization:
            ...
            pred2 = torch.sigmoid(mid_output2.view(mid_output2.shape[0] * mid_output2.shape[1]))
            # print(f"shape = {mid_output.shape}")
            # print(f"aview = {self.adj_mx.view(mid_output.shape[0] * mid_output.shape[1])}")
            true_label1 = self.adj_mx.view(mid_output1.shape[0] * mid_output1.shape[1]).to(self.device)
            true_label2 = self.adj_mx.view(mid_output2.shape[0] * mid_output2.shape[1]).to(self.device)
            compute_loss = torch.nn.BCELoss()
            loss_g1 = compute_loss(pred1, true_label1)
            loss_g2 = compute_loss(pred2, true_label2)
            self._logger.debug(f"loss_g1 = {loss_g1}, loss_g2 = {loss_g2}, loss_1 = {loss_1}")
            loss_t = loss_1 + (loss_g1 + loss_g2)/2
            return loss_t
        else:
    ...
    def updateAdj(self, adj_mx, adj_mx2):
        self.adj_mx1=adj_mx
        self.adj_mx2=adj_mx2
